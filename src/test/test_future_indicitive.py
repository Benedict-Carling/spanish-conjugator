...
def test_future_indicative_nosotros_ar():
    expected = "hablaremos"
    assert (
        str(Conjugator().conjugate("hablar", "future", "indicative", "nosotros"))
        == expected
    )
...
def test_future_indicative_vosotros_ar():
    expected = "hablaréis"
    assert (
        Conjugator().conjugate("hablar", "future", "indicative", "vosotros") == expected
    )
...
def test_future_indicative_ustedes_ar():
    expected = "hablarán"
    assert (
        Conjugator().conjugate("hablar", "future", "indicative", "ustedes") == expected
    )
...
def test_future_indicative_ustedes_second_exp():
    expected = "charlarán"
    assert (
        Conjugator().conjugate("charlar", "future", "indicative", "ustedes") == expected
    )
